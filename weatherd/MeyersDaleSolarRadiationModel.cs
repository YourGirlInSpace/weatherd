...
using Angle = weatherd.models.Angle;
...
namespace weatherd
{
    public class MeyersDaleSolarRadiationModel
    {
        ...
        public MeyersDaleSolarRadiationModel(Angle latitude, Angle longitude)
        {
            ...
            Serilog.Log.Information("Meyers-Dale solar radition model parameters:  Lat={latitude}, Lon={longitude}",
                            latitude, longitude);
        }
        ...
        public Irradiance CalculateSolarRadiation(DateTime now)
        {
            ...
            var lambda = CalculateLambda(now, _latitude);

            var solarZenith = Angle.Pos90 - Sun.CalculateElevation(now, _latitude, _longitude);
            var m = 35 * Pow(1224 * Pow(solarZenith.Cos(), 2) + 1, -0.5);
            var u = Exp(0.1133 - Log(lambda + 1) + 0.0393 * Td);
            var TrTg = 1.021 - 0.084 * Pow(m * (949 * p * Pow(10, -5) + 0.051), 0.5);
            var Tw = 1 - 0.077 * Pow(u * m, 0.3);
            var Ta = Pow(x, m);
        ...
        private static Irradiance CalculateAtmosphericTopSolarInsolation(DateTime time)
        {
            ...
            return new Irradiance(Isc * (1.00011 + 0.034221 * theta.Cos() + 0.00128 * theta.Sin() - 0.000719 * (2 * theta).Cos() +
