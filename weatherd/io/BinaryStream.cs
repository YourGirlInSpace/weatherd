...
namespace weatherd.io
{
    ...
    public class BinaryStream : IDisposable
    {
        ...
        private static Endianness GetSystemEndianness() => BitConverter.IsLittleEndian ? Endianness.Little : Endianness.Big;
        ...
        {
            int bytesRead = _stream.Read(_buffer, 0, count);
            if (bytesRead < count)
        ...
        public virtual string ReadString(int length)
        {
            ...
            while (n < length)
            {
                ...
                {
                    if (_stream.CanSeek)
                        _stream.Seek(opos - _stream.Position, SeekOrigin.Current);
                    throw;
                }
        ...
        public virtual void Write(byte[] buffer, int start, int count, Endianness endianness)
        {
            ...
            _stream.Write(_buffer, start, count);
        }
        ...
        public virtual void Skip(long bytes) => _stream.Seek(bytes, SeekOrigin.Current);
        ...
        public byte[] ToArray()
        {
            ...
            byte[] b = new byte[_stream.Length];
